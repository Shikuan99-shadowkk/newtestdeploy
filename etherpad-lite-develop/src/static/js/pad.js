      ...
      setTimeout(() => {
        ...
        liNode.onclick = () => {
          if (!echartsIsShow) {
            ...
            var elements = innerIframe.document.querySelectorAll(
              '[class^="author-a-"]'
            );
            // 创建一个空数组来存储类名
            var classNames = [];
            ...
            function getTotalTextLengthByClass(className) {
              var elements =
                innerIframe.document.getElementsByClassName(className); // 根据类名获取元素列表
              var totalLength = 0;
            ...
            let otherFontNum = getTotalTextLengthByClass("innerdocbody");
            let clientList = clientVars.collab_client_vars.historicalAuthorData;
            for (let i = 0; i < uniqueClassNames.length; i++) {
              otherFontNum =
                otherFontNum - getTotalTextLengthByClass(uniqueClassNames[i]);
              dataArr[i] = {
                ...
                colorId:
                  typeof clientList[getAuthorFromClassName(uniqueClassNames[i])]
                    .colorId == "number"
                    ? clientVars.colorPalette[
                        clientList[getAuthorFromClassName(uniqueClassNames[i])]
                          .colorId
                      ]
                    : clientList[getAuthorFromClassName(uniqueClassNames[i])]
                        .colorId,
                name: clientList[getAuthorFromClassName(uniqueClassNames[i])]
                  .name,
              };
            }
            // if (otherFontNum > 0) {
            ...
            chartContainer.style.bottom = "10px";
            chartContainer.style.width = "300px";
            chartContainer.style.height = "300px";
            document.body.appendChild(chartContainer);
            window.console.log(chartContainer);
          ...
          echartsIsShow = !echartsIsShow;
        };
        btnlistDom.appendChild(liNode);
      }, 100);
    };
